 ...
 *
 * <p>Verifies that the gross, commission, and total are computed correctly for
 * both buy and sell orders, including the following properties:</p>
 * <ul>
 *   <li>Gross = price × quantity</li>
 *   <li>Commission = gross × {@link CostPreviewCalculator#COMMISSION_RATE} (0.5 %)</li>
 *   <li>Buy total = gross + commission</li>
 *   <li>Sell total = gross − commission</li>
 * </ul>
 */
class CostPreviewCalculatorTest {
  ...
  void constructor_buy_grossIsProductOfPriceAndQuantity() {
    CostPreviewCalculator calc = new CostPreviewCalculator(
        new BigDecimal("100"), new BigDecimal("10"), true);
    assertEquals(0, new BigDecimal("1000").compareTo(calc.getGross()),
        "Expected gross = 100 × 10 = 1000");
  }
  ...
  void constructor_buy_commissionIsHalfPercentOfGross() {
    CostPreviewCalculator calc = new CostPreviewCalculator(
        new BigDecimal("100"), new BigDecimal("10"), true);
    assertEquals(0, new BigDecimal("5").compareTo(calc.getCommission()),
  ...
  void constructor_buy_totalIsGrossPlusCommission() {
    CostPreviewCalculator calc = new CostPreviewCalculator(
        new BigDecimal("100"), new BigDecimal("10"), true);
    assertEquals(0, new BigDecimal("1005").compareTo(calc.getTotal()),
  ...
  void constructor_sell_totalIsGrossMinusCommission() {
    CostPreviewCalculator calc = new CostPreviewCalculator(
        new BigDecimal("100"), new BigDecimal("10"), false);
    assertEquals(0, new BigDecimal("995").compareTo(calc.getTotal()),
  ...
  void constructor_sell_commissionEqualsCommissionForBuy() {
    CostPreviewCalculator buy  = new CostPreviewCalculator(
        new BigDecimal("200"), new BigDecimal("4"), true);
    CostPreviewCalculator sell = new CostPreviewCalculator(
        new BigDecimal("200"), new BigDecimal("4"), false);
    assertEquals(0, buy.getCommission().compareTo(sell.getCommission()),
  ...
  void constructor_singleShare_buy_correctBreakdown() {
    ...
    CostPreviewCalculator calc = new CostPreviewCalculator(
        new BigDecimal("50"), new BigDecimal("1"), true);
