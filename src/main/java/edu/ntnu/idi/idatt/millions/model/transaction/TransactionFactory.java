 ...
 *
 * <p>This class follows the <em>Factory Method</em> design pattern. Subclasses define which
 * concrete {@link Transaction} type to instantiate by implementing {@link #create(Share, int)},
 * while this base class provides the shared {@link #createAndCommit(Share, int, Player)} behaviour
 * that creates a transaction and immediately commits it.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public abstract class TransactionFactory {
   ...
   *
   * <p>Subclasses decide which concrete {@link Transaction} type to return.
   *
   ...
   *
   * <p>This method delegates creation to {@link #create(Share, int)} and then calls {@link
   * Transaction#commit(Player)}, so both steps are always performed together without duplicating
   ...
   * @throws IllegalArgumentException if {@code week} is not positive
   * @throws IllegalStateException if the transaction cannot be committed (e.g. insufficient funds
   *     or missing share)
   */
