...
import edu.ntnu.idi.idatt.millions.model.portfolio.Holding;
...
/**
 * Section panel that displays the player's aggregated stock holdings as a paginated {@link
 * TableView}. Each row represents one distinct stock symbol and shows total quantity, weighted buy
 * price, current price, total value, unrealised gain/loss, and a Quick Sell button.
 *
 * <p>Column headers and numeric values switch to compact short-form when the table width drops
 * below {@link TableStyleUtils#COMPACT_THRESHOLD} so the panel remains readable at narrow viewport
 * widths.
 */
public class HoldingsPanel extends VBox {
  ...
  /** Constructs the holdings panel and builds its layout as a section card. */
  ...
  private TableView<Holding> buildHoldingsTable() {
    ...
    TableColumn<Holding, Holding> stockCol =
        TableStyleUtils.stockIconColumn(h -> h.stock().getSymbol(), h -> h.stock().getCompany());
    TableColumn<Holding, BigDecimal> qtyCol =
        buildHoldingValueCol(
            "Quantity",
            Holding::totalQuantity,
            ViewFormatter::quantity,
            ViewFormatter::quantity,
            table);
    TableColumn<Holding, BigDecimal> buyCol =
        buildHoldingValueCol(
            "Buy Price",
            Holding::weightedBuyPrice,
            ViewFormatter::price,
            ViewFormatter::wholePrice,
            table);
    TableColumn<Holding, BigDecimal> currCol =
        buildHoldingValueCol(
            "Current Price",
            h -> h.stock().getSalesPrice(),
            ViewFormatter::price,
            ViewFormatter::wholePrice,
            table);
  ...
  /**
   * Builds a sortable numeric column that extracts a {@link BigDecimal} value from each {@link
   * Holding} and switches between a long and compact formatter at the {@link
   * TableStyleUtils#COMPACT_THRESHOLD} width breakpoint.
   *
            ...
            new TableCell<>() {
              ...
              protected void updateItem(BigDecimal item, boolean empty) {
                ...
                if (empty || item == null) {
                  ...
                  setText(
                      (table.getWidth() < TableStyleUtils.COMPACT_THRESHOLD
                              ? shortFormatter
                              : longFormatter)
                          .apply(item));
                }
  ...
  /**
   * Builds the Gain / Loss column with profit/loss colouring via CSS classes. Uses compact {@code
   * ±$1234} format when the table is narrow.
   *
   * @param table the parent table, used to read the current width
            ...
            new TableCell<>() {
              ...
              protected void updateItem(BigDecimal item, boolean empty) {
                ...
                if (empty || item == null) {
                  ...
                  getStyleClass()
                      .add(item.signum() >= 0 ? "table-data-cell-profit" : "table-data-cell-loss");
                }
              }
            });
    col.setMinWidth(80);
  ...
  /**
   * Builds the Quick Sell action column. Clicking the button invokes the registered {@link #onSell}
   * handler with the row's symbol and total quantity. The button label shortens to "QS" in compact
   * mode.
   *
            ...
            new TableCell<>() {
...

              {
                ...
                btn.textProperty()
                    .bind(
                        Bindings.when(
                                table.widthProperty().lessThan(TableStyleUtils.COMPACT_THRESHOLD))
                            .then("QS")
  ...
  /**
   * Adds a width listener that calls {@link TableView#refresh()} whenever the table crosses the
   * {@link TableStyleUtils#COMPACT_THRESHOLD}, so cells re-render with the appropriate formatter.
   */
  ...
  /**
   * Replaces the holdings table items with the given list.
   *
  ...
  /**
   * Registers a handler invoked when the user clicks Quick Sell. Receives the stock symbol and the
