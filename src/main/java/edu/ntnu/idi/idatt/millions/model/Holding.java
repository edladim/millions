...
/**
 * <p>
 * Aggregated, immutable view of all {@link Share} lots that belong to the same
 * underlying {@link Stock}. A {@code Holding} sums the quantities and totals of
 * the lots and computes the quantity-weighted average purchase price, so views
 * can render a single row per stock without recomputing the aggregation on each
 * cell access.
 * </p>
 *
...
public record Holding(
  ...
  /**
   * <p>Builds a {@code Holding} from a non-empty list of {@link Share} lots that
   * all reference the same stock. The weighted buy price is
   * {@code totalInvestment / totalQuantity}; if the total quantity is zero, the
   * weighted price falls back to {@link BigDecimal#ZERO} to avoid division by
   * zero.</p>
   *
  ...
  public static Holding aggregate(List<Share> lots) {
    ...
    BigDecimal totalInvestment = lots.stream()
        ...
        .reduce(BigDecimal.ZERO, BigDecimal::add);
    BigDecimal weightedBuyPrice = totalQuantity.signum() == 0
        ? BigDecimal.ZERO
  ...
  /**
   * <p>Returns the current market value of this holding,
   * {@code currentPrice × totalQuantity}.</p>
   *
  ...
  /**
   * <p>Returns the unrealised gain or loss for this holding,
   * {@code totalValue − totalInvestment}.</p>
   *
