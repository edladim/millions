...
/**
 * Utility for computing the player's "Millions Score" — a single number that rewards both <em>how
 * fast</em> the player grew their capital and <em>how long</em> they survived in the market.
 *
 * <p>The score is the sum of two components:
 *
 * <ul>
 *   <li><b>Performance</b> — based on the annualised return rate, so a player who doubles their
 *       capital in one year scores far higher than one who doubles over ten years.
 *   <li><b>Survival</b> — a logarithmic bonus for weeks played, scaled by how much of the starting
 *       capital the player preserved. A long career that weathered downturns still earns a
 *       meaningful score even if the annualised rate is modest, but losing most of the capital
 *       reduces the bonus proportionally — surviving with empty pockets isn't really surviving.
 * </ul>
 *
 * <p>A floor of {@value #MIN_WEEKS_FOR_ANNUALISATION} weeks is applied when annualising so a single
 * lucky week cannot inflate the rate to an absurd value. Players who lose everything ({@code
 * returnRate <= -100%}) score 0.
 *
...
public final class ScoreCalculator {
  ...
  /**
   * Minimum effective week count used when annualising, to prevent very short games from inflating
   * the rate.
   */
  ...
  /**
   * Computes the Millions Score from the player's total return rate and the number of weeks they
   * played.
   *
   * <p>Formula:
   *
   ...
   * </pre>
   *
   * @param returnRate the player's total return rate as a decimal (e.g. {@code 0.20} for a 20%
   *     gain)
