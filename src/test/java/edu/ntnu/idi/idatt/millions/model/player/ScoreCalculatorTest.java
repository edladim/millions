 ...
 *
 * <p>Verifies the scoring formula including:</p>
 * <ul>
 *   <li>Wipe-out edge case ({@code returnRate <= -1.0} gives zero)</li>
 *   <li>Non-negativity guarantee</li>
 *   <li>Higher returns produce higher scores</li>
 *   <li>More weeks produce higher survival bonuses</li>
 *   <li>The minimum-weeks clamp prevents short-game inflation</li>
 *   <li>Known exact values (hand-computed from the formula)</li>
 * </ul>
 */
class ScoreCalculatorTest {
  ...
  /**
   * Verifies that a zero return with no weeks played scores zero.
   * (No growth → zero performance; no weeks → zero survival bonus.)
   */
  @Test
  ...
  void compute_higherReturn_givesHigherScore() {
    ...
    assertTrue(highScore > lowScore,
        "Expected 100% return to score higher than 20% return");
  }

  /**
   * Verifies that more weeks produce a higher survival bonus when the return rate is zero.
   *
   * <p>With zero return the performance component is always zero, so the difference
   * in score is driven entirely by the survival bonus, which grows with weeks played.</p>
   */
  @Test
  void compute_moreWeeks_givesHigherSurvivalBonus() {
    ...
    assertTrue(longGame > shortGame,
        "Expected longer game to score higher than shorter game at the same zero return rate");
  }

  /**
   * Verifies that the minimum-weeks clamp prevents a short game from scoring
   * <em>higher</em> than the equivalent 26-week game.
   *
   * <p>The clamp applies only to the annualisation (performance component);
   * the survival component still uses the actual week count. Therefore a 1-week
   * game must score lower than a 26-week game because it has the same performance
   * score but a smaller survival bonus.</p>
   */
  @Test
  void compute_shortGameClampedToMinWeeks_doesNotInflateAbove26Weeks() {
    ...
    assertTrue(oneWeek < twentySixWeeks,
  ...
  /**
   * Verifies that a large loss (but not a full wipe-out) gives a lower score
   * than break-even, due to the reduced preservation factor.
   */
  @Test
  void compute_largeLoss_scoresLowerThanBreakEven() {
    ...
    assertTrue(bigLoss < breakEven,
        "Expected large loss to score lower than break-even");
  }
}
