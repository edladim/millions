...
/**
 * Section panel that displays the player's transaction history as a paginated {@link TableView},
 * sorted most-recent-first by default. Rows are colour-coded by type: buy rows receive the {@code
 * tx-row-buy} CSS class and sell rows receive {@code tx-row-sell}.
 *
 * <p>The Quantity column header shortens to "QTY" at narrow viewport widths (below {@link
 * TableStyleUtils#COMPACT_THRESHOLD}).
 */
public class TransactionHistoryPanel extends VBox {
  ...
  /**
   * Builds a sortable {@link BigDecimal} column for the transaction table.
   *
  ...
  /**
   * Builds the Type column. Cell text is "Buy" or "Sell" coloured via {@code tx-type-buy} / {@code
   * tx-type-sell} CSS classes.
   *
  ...
  /**
   * Builds the Week column, displaying values as {@code "W<n>"} (e.g. {@code W12}).
   *
  ...
  /**
   * Replaces the transaction history table items with the given list.
   *
